...
# backing store for the dictionary
var _wheel_node: Dictionary = {}

var WheelNode: Dictionary:
	set(value):
		_wheel_node = value   # just store safely
	get:
		return _process_wheel(_wheel_node)  # process whenever you read
	   # return stored data, not WheelNode itself

var BindedTarget

func _process_wheel(data: Dictionary) -> Dictionary:
	# Do your custom logic here
	# e.g., add keys, modify values, validate, etc.
	var processed = data.duplicate()
	processed["last_accessed"] = Time.get_ticks_msec()
	return processed
...
func _on_UseAsTraction_pressed():
	for Wheel in WheelNode:	WheelNode[Wheel].use_as_traction = int(get_node("General/UseAsTraction").is_pressed())

func _on_UseAsSteering_pressed():
	for Wheel in WheelNode:	WheelNode[Wheel].use_as_steering = int(get_node("General/UseAsSteering").is_pressed())
