...
func _process(delta):
	...
	if(camera_onoff):
		...
		if(cameraTimer > 0):
			...
			cameraOrbit.rotation_degrees.x = lerpf(cameraOrbit.rotation_degrees.x, followCameraAngle, delta * 10.0)
			cameraOrbit.rotation_degrees.y = lerpf(cameraOrbit.rotation_degrees.y, followCameraY, delta * 10.0)
